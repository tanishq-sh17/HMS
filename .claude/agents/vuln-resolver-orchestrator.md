...
$CONFIG_PATH = "$REPO_ROOT\.claude\config\ghas-w2-config.yml"

python "$REPO_ROOT\.claude\scripts\validate_config.py" $CONFIG_PATH
...
## Step 1 — @w2-context-builder (`model: haiku`)
...
## Step 3 — @w2-planner (`model: sonnet`) (first call)
...
## Step 6 — @w2-verifier (`model: haiku`)
...
## Step 9 — @w2-reporter (`model: haiku`)
...
## Rules
- Spawn sub-agents with `agent_type: "general-purpose"` and `model` matching each agent's frontmatter annotation (see per-step labels) — never use custom agent types
